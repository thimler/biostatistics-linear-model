...
Our data are presented as a dataframe of 119 samples (rows) and 5 colums (presented above) : 
...
interaction1 <- ggplot(trees, aes(x = Species, y = Moisture, color = Transpiration)) + geom_point() +
  stat_summary(fun.y = mean, geom = "line", aes(group = Transpiration), size = 1) + theme_bw()

interaction2 <- ggplot(trees, aes(x = Species, y = Moisture, color = Branch)) + geom_point() +
  stat_summary(fun.y = mean, geom = "line", aes(group = Branch), size = 1) + theme_bw()
...
interaction4 <- ggplot(trees, aes(x = Species, y = Moisture, color = Location)) + geom_point() +
  stat_summary(fun.y = mean, geom = "line", aes(group = Location), size = 1) + theme_bw()
...
First of all, to test an ANOVA analysis, we have to test some assumption: measures in each population are normally distributed, variances in each population are equal (homoscedasticity) and samples are obtained independently at random from the populations.
...
Here, we can have several models to for our data:
...
We have tested some comparison of models:
...
As we can see, the value number `r which.max(aov.trees.opt$residuals)` could be an outlier but its Cook's distance is arround 0.8 so we can keep it.
